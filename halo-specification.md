...
To maintain clean architecture and proper separation of concerns, an MCP server should be a standalone service — its own codebase, its own repository, its own deployment pipeline, its own infrastructure. There are legitimate cases where this makes sense — an MCP server that reshapes the API surface for LLM consumption, combines multiple backend calls into a single tool, or presents a fundamentally different abstraction to agents than the underlying API exposes. In these cases, the MCP server is doing real work that justifies its existence as a separate service.

In practice, however, many MCP servers are thin wrappers — they map directly to the underlying API endpoints with minimal transformation. For these, the standalone service is pure operational overhead with no value added beyond protocol translation. HALO specifically targets this space: APIs that are already HTTP endpoints and need to be discoverable by LLM agents without an intermediate service layer.
